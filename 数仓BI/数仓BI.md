...
- [2. 数仓概念](#2-数仓概念)
    ...
    - [2.2. 说说数仓(2) - 传统数仓与互联网数仓](#22-说说数仓2---传统数仓与互联网数仓)
        ...
        - [2.2.4. 数据平台第一代架构（数据源 -> EDW -> 分析服务器出结果）没有多级的ETL](#224-数据平台第一代架构数据源---edw---分析服务器出结果没有多级的etl)
        - [2.2.5. 数据平台第二代架构（数据源 -> EDS -> DM -> 结果呈现）多级ETL](#225-数据平台第二代架构数据源---eds---dm---结果呈现多级etl)
        - [2.2.6. 数据平台第三代架构 - 数据仓库混合性架构 (Cif)](#226-数据平台第三代架构---数据仓库混合性架构-cif)
        - [2.2.7. 新一代架构 OPDM 操作型数据集市 (仓库)](#227-新一代架构-opdm-操作型数据集市-仓库)
        - [2.2.8. 数据模型（概念模型、逻辑模型、物理模型）](#228-数据模型概念模型逻辑模型物理模型)
        - [2.2.9. 数据源差异（非互联网、互联网、移动互联网）](#229-数据源差异非互联网互联网移动互联网)
        - [2.2.10. 约 2010-2012 年的平台结构](#2210-约-2010-2012-年的平台结构)
        - [2.2.11. 约 2012-2013 年的平台结构](#2211-约-2012-2013-年的平台结构)
        - [2.2.12. 事实表、维度举例](#2212-事实表维度举例)
        - [2.2.13. 雪花模型&星型模型](#2213-雪花模型星型模型)
            - [2.2.13.1. 先举个例子，淘宝商品的雪花模型](#22131-先举个例子淘宝商品的雪花模型)
            - [2.2.13.2. 概述](#22132-概述)
                - [2.2.13.2.1. 销售数据仓库中的星型模型](#221321-销售数据仓库中的星型模型)
                - [2.2.13.2.2. 销售数据仓库中的雪花模型](#221322-销售数据仓库中的雪花模型)
            - [2.2.13.3. 使用选择](#22133-使用选择)
- [3. 数仓技术实现](#3-数仓技术实现)
    - [3.1. 元数据管理要义](#31-元数据管理要义)
    - [3.2. 分层逻辑架构](#32-分层逻辑架构)
    - [3.3. 阿里数仓分层](#33-阿里数仓分层)
    - [3.4. 阿里模型架构图 （大数据之路9.3）](#34-阿里模型架构图-大数据之路93)
- [4. 数据管理篇](#4-数据管理篇)
    - [4.1. 元数据](#41-元数据)
...
### 2.2.4. 数据平台第一代架构（数据源 -> EDW -> 分析服务器出结果）没有多级的ETL
（开发时间 2001-2002 年）
海尔集团的一个 BI 项目，架构的 ETL 使用的是 微软的数据抽取加工工具 DTS
![](img/2019-09-20-17-17-16.png)
### 2.2.5. 数据平台第二代架构（数据源 -> EDS -> DM -> 结果呈现）多级ETL
...
![雪花模型](img/2019-09-25-17-45-33.png)
相比较而言，星形模型实用的是反规范化数据。在星形模型中，维度直接指的是事实表，业务层级不会通过维度之间的参照完整性来部署。
![星型模型](img/2019-09-25-17-45-48.png)
+ 业务模型
